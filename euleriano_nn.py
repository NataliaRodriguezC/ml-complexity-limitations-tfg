...
def euleriano(entrada):
    ...
    for bit, (u, v) in zip(entrada, edge_list):
        if bit == 1:
            adj[u].append(v)
            adj[v].append(u)
    
    grados = {i: len(adj[i]) for i in range(n)}
    
    for i in range(n):
        if grados[i] % 2 != 0:
            return 0
    
    if sum(grados.values()) == 0:
        return 1
    
    start = next(i for i in range(n) if grados[i] > 0)
    visitados = [0] * n
    stack = [start]
    ...
    for i in range(n):
        if grados[i] > 0 and not visitados[i]:
            return 0
...
def red_neuronal(model):
    ...
    y = np.array([euleriano(row) for row in X], dtype=np.float32)
...
sensitivity_stats(euleriano, 21)
