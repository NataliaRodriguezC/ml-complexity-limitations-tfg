...
def clique(entrada):
    ...
    for bit,(u,v) in zip(graph_bits, edges):
        if bit: 
            adj[u][v] = adj[v][u] = True
    
    for r in range(k, n+1):
        for subset in itertools.combinations(range(n), r):
            if all(adj[u][v] for u,v in itertools.combinations(subset,2)):
                return True
    return False
...
def red_neuronal(model):
    ...
    y = np.array([clique(row) for row in X], dtype=np.float32)
